...
namespace Runtime.Utilities
{
    public static class TransformUtility
    {
        ...
        public static List<Vector2> GetPositionsAroundPoint(Vector2 centerPoint, int numberOfPositions = 10, float space = 1.0f)
        {
            var positions = new List<Vector2>();
            var angle = 0.0f;
            var deltaAngle = 360.0f / numberOfPositions;
            for (int i = 0; i < numberOfPositions; i++)
            {
                float angleInRadian = angle * Mathf.Deg2Rad;
                var direction = new Vector2(Mathf.Cos(angleInRadian), Mathf.Sin(angleInRadian));
                var spawnPoint = centerPoint + direction * space;
