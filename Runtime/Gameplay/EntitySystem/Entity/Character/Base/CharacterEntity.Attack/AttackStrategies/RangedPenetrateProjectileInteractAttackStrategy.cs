...
{
    public class RangedPenetrateProjectileInteractAttackStrategy : RangedInteractAttackStrategy
    {
        ...
        private const string AUDIO_SKILL1_KEY = "sfx_skill_penetrating_throw";
        ...
        private async UniTaskVoid SpawnProjectilesAsync(IInteractable interactableTarget, CancellationToken cancellationToken)
        {
            ...
            if (_isTargetPoisoned)
            {
                ...
                length = _damageSpears.Length;

                await AudioController.Instance.PlaySoundEffectAsync(AUDIO_SKILL1_KEY, cancellationToken);
            }
            else
            {
